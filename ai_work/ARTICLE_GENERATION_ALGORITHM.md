...
**Description**: How "unpredictable" text is to models
...
- Self-interruptions ("But first... no, let me tell you correctly...")
...
- Anachronisms (rarely): "iseche", "ekhat"
- Local expressions
...
**Bad**:
```
I came home. Opened the door. Went inside. Sat together.
```

**Good**:
```
Door. Opened. Then I entered the room where mother, father and grandma sat.
```

---

#### 3. Colloquialism (>= 75)
**Description**: Conversational expression proportion
...
**Description**: Emotional depth and authenticity
...
**Description**: Incomplete, fragmented sentences like live speech
...
**Description**: Natural word/phrase repetition like remembering
...
### Example: Content Orchestrator

```typescript
export class ContentOrchestrator {
  async generateArticle(params: ArticleParams): Promise<Article> {
    // Stage 0: PlotBible
    const plotBible = await this.stage0.generatePlotBible(params);

    // Stage 1: Episodes with auto-restore
    const episodes = [];
    for (let i = 0; i < plotBible.episodes.length; i++) {
      let episode = await this.stage1.generateEpisode(plotBible, i);
      
      // Auto-restore (max 3 attempts)
      for (let attempt = 0; attempt < 3; attempt++) {
        const phase2Score = this.phase2Scorer.calculate(episode.content);
        if (phase2Score >= 70) break;
        episode = await this.stage1.autoRestore(episode);
      }
    ...
    // Stage 2: Assembly
    let article = await this.stage2.assembleArticle(episodes);

    // Stage 3: Voice Restoration with auto-restore
    for (let attempt = 0; attempt < 2; attempt++) {
      article = await this.stage3.restoreVoice(article);
      const phase2Score = this.phase2Scorer.calculate(article.content);
      if (phase2Score >= 85) break;
      
      // If failed after 2 attempts, regenerate Stage 2
      if (attempt === 1) {
        return this.generateArticle(params);
      }
    }

    // Stage 4: Anti-Detection
    const antiDetectionResult = await this.stage4.applyAntiDetection(article);
    if (antiDetectionResult.overallScore < 80) {
      // Return to Stage 3 (re-apply voice only)
      return this.stage3.retryVoice(article);
    }
    article = antiDetectionResult.processedArticle;

    // Stage 5: Quality Checklist
    const checklistResult = await this.stage5.runChecklist(article);
    if (checklistResult.score < 8) {
      return this.stage3.retryVoice(article);
    }
