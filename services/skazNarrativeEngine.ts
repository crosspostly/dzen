...
export class SkazNarrativeEngine {
  ...
  // Обращения к читателю (Разрушение четвертой стены - Hostile Elegance)
  private readerAppeals: string[] = [
    "Знаете, в чем главная ошибка?",
    "Я смотрела на это и чувствовала только ледяное спокойствие.",
    "Вы когда-нибудь задумывались, почему мы добровольно выбираем это рабство?",
    "Давайте будем честными, хотя бы в этом цифровом пространстве.",
    "Это не просто история, это анатомия системного инфантилизма.",
    "Мой интеллектуальный скальпель готов, начнем препарирование.",
    "Вы все еще верите в романтические мифы? Очень зря.",
    "Я использую метод ледяного зеркала: посмотрите на себя моими глазами.",
  ];

  // Элементы "запинки" и подбора слов (в стиле Hostile Elegance)
  private hesitations: string[] = [
    "...как бы это препарировать точнее...",
    "...понимаете, о какой нищете духа я говорю...",
    "...ну, в общем, вы узнали в этом своего 'героя'...",
    "...даже не знаю, как описать этот уровень деградации...",
    "...если вы уже дошли себя, вы понимаете...",
    "...это типичный ресурсный дефицит, понимаете?...",
  ];

  // 🔄 ZenMaster v5.0: Hostile Elegance & Intellectual Scalpel
  private dialectalWords: Map<string, string[]> = new Map([
    ["искать", ["разыскивать", "препарировать", "анализировать"]],
    ["смотреть", ["наблюдать", "разглядывать", "препарировать взглядом", "вглядываться в суть"]],
    ["идти", ["двигаться к себе", "пробираться сквозь рутину", "направляться", "совершать паломничество"]],
    ["говорить", ["декларировать", "высказывать", "излагать", "чеканить слова"]],
    ["плохо", ["скверно", "деструктивно", "токсично", "инфантильно"]],
    ["очень", ["крайне", "весьма", "исключительно", "критически"]],
    ["интересно", ["любопытно", "занятно", "примечательно", "диагностически важно"]],
  ]);

  // Неологизмы стратегии (СЕМАНТИЧЕСКИЙ ЩИТ)
  private neologisms: Map<string, string[]> = new Map([
    ["найти себя", ["дойти себя", "дойти до своей сути"]],
    ["стать сильнее", ["ковать в себе стальной стержень", "ковать свою автономность"]],
    ["энергия", ["сексофон", "внутренний драйв"]],
    ["манипуляция", ["мужеловка", "поведенческий паттерн"]],
    ["токсичность", ["мозгоёлка", "энергетический вампиризм"]],
    ["нежность", ["пронежность", "осознанная уязвимость"]],
    ["возраст", ["состояние бездаты", "благодатная"]],
  ]);

  // Стандартные конструкции, которые нужно переделать (синтаксическое нарушение + ФИКС ГЕНДЕРА)
  private syntacticTransformations: Array<[RegExp, string]> = [
    ...
    [/я понял\b/gi, "поняла я"],
    [/я решил\b/gi, "решила я"],
    [/я вышел\b/gi, "вышла я"],
    [/я посмотрел\b/gi, "посмотрела я"],
  ];
  ...
  public applySkazTransformations(text: string): string {
    ...
    // 5. Инъекция неологизмов стратегии
    result = this.injectNeologisms(result);

    // 6. Добавляем диалектные слова
  ...
  /**
   * Инъекция неологизмов стратегии
   */
  private injectNeologisms(text: string): string {
    let result = text;

    const entries = Array.from(this.neologisms.entries());
    for (const [standard, replacements] of entries) {
      const pattern = new RegExp(`\\b${standard}\\b`, "gi");
      const matches = text.match(pattern);
      
      if (matches && matches.length > 0) {
        // Заменяем 30-50% вхождений
        let replaced = 0;
        const replaceCount = Math.floor(matches.length * (0.3 + Math.random() * 0.2));
        
        result = result.replace(pattern, () => {
          if (replaced < replaceCount) {
            replaced++;
            return this.selectRandomElement(replacements);
          }
          return standard;
        });
      }
