...
export class FinalArticleCleanupGate {
  ...
  /**
   * 🎯 DEEP TEXT RESTORATION - УПРОЩЁННАЯ ВЕРСИЯ (v6.1)
   * Минимальный промпт для быстрой публикации статьи
   */
  private async callGeminiForCleanup(article: string, analysis: IssueAnalysis): Promise<string> {
    const deepRestorationPrompt = `Перепиши статью для публикации на Yandex Zen.

ПРАВИЛА:
1. Удали всё лишнее: [note], [comment], [scene], (текст в скобках), **жирный**, ## заголовки
2. Исправь сломанные диалоги: "— Кто это? я," → "— Кто это? — спросил я."
3. Разбей длинные предложения (более 50 слов) на короткие
4. Убери повторяющиеся фразы ("— вот в чём дело" и т.п.) - максимум 1-2 раза
5. Убери частицы-паразиты в начале: "ну и", "да вот", "вот только"
6. Сохрани сюжет, диалоги, метафоры, стиль автора
7. Структурируй абзацы: короткий → средний → короткий → длинный

РЕЗУЛЬТАТ: Только чистый текст статьи, готовый к публикации.
В конце добавь: ✅ DEEP RESTORATION COMPLETE

${article}

ПЕРЕПИШИ ТОЛЬКО ТЕКСТ, БЕЗ КОММЕНТАРИЕВ И ПОЯСНЕНИЙ:`;
