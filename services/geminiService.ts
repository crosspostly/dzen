    ...
    try {
      ...
      return {
        ...
        metadata: {
          ...
          model_used: config.gemini_model || 'gemini-3.1-flash-lite-preview',
        },
      };
    } catch (error) {
...
60-100 = человеческий текст (вариативный, живой, эмоциональный)`;
  ...
  public async callGemini(params: {
    ...
    try {
      ...
      // 🔄 ФОЛБЕК: если модель перегружена, используем gemini-3.1-flash-lite-preview
      if (errorMessage.includes('503') || errorMessage.includes('overloaded') || errorMessage.includes('UNAVAILABLE')) {
        console.log(`🔄 Model overloaded, trying fallback to gemini-3.1-flash-lite-preview...`);

        try {
          const fallbackResponse = await this.ai.models.generateContent({
            model: "gemini-3.1-flash-lite-preview", // 🔥 ФОЛБЕК МОДЕЛЬ
            contents: prompt,
            config: {
              temperature,
              topK: 40,
