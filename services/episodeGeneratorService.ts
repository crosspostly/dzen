...
⚠️  БЕЗ ПОВТОРЯЮЩИХСЯ ФРАЗ-ПАРАЗИТОВ (максимум 1-2 раза на ВСЮ статью):
   ...
   ✅ Варьируй речевые обороты! Используй разные фразы для передачи мыслей.

⚠️  БЕЗ ORPHANED ФРАГМЕНТОВ (разорванные предложения в начале):
   ❌ "ну и" в начале предложения
   ❌ "да вот" в начале предложения
   ❌ "вот только", "вот это", "и то" в начале
   ❌ "хотя", "потому что", "но вот" в начале
   
   ✅ Пиши полные, законченные предложения с естественным началом!
...
⚠️  ПЕРЕД ОТВЕТОМ - ФИНАЛЬНАЯ ПРОВЕРКА (v6.0):

Перечитай свой текст и убедись что:

☐ НЕТ метаданных: [note], [comment], [TODO], [...] → УДАЛИ!
☐ НЕТ markdown: **, ##, ___, \`\`\` → УДАЛИ!
☐ НЕТ повторяющихся фраз > 2 раз:
   - "— вот в чём дело" → макс 1-2 раза
   - "— одним словом" → макс 1-2 раза
   - "— может быть, не совсем точно, но" → УДАЛИ полностью!
☐ НЕТ orphaned фрагментов в начале предложения:
   - "ну и", "да вот", "вот только" → ПЕРЕПИСАТЬ!
