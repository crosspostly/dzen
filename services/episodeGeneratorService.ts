...
/**
   * 🎬 Episode Generator Service v7.0 (ANTI-ARTIFACT UPGRADE)
   *
   ...
   *
   * v7.0 CHANGES - CRITICAL ANTI-ARTIFACT RULES:
   * - ✅ ZERO TOLERANCE for orphaned fragments at paragraph starts
   * - ✅ MAXIMUM 1 repetition per phrase (was 2): "вот что я хочу сказать", "одним словом"
   * - ✅ STRICT formatting for merged words: "Маринасидела" → split properly
   * - ✅ ENHANCED examples of WRONG vs CORRECT text in prompt
   * - ✅ ADDED "runaway phrases" detection: phrases repeated 50+ times
   * - ✅ IMPROVED broken sentence detection at paragraph starts
   * - ✅ STRONGER validation in final instruction section
   *
   * Previous v4.6 features remain:
...
⚠️  БЕЗ НИКАКИХ комментариев, метаданных, пометок в квадратных скобках:
   ❌ [note: ...], [comment: ...], [TODO: ...], [EDITOR: ...]
   ❌ Любые [...] в тексте статьи

⚠️  БЕЗ markdown синтаксиса:
   ❌ **жирный текст**, ##заголовки, ___ разделители, \`\`\`код\`\`\`
   ✅ Только чистый текст с диалогами через em-dash (—)
...
⚠️  БЕЗ ПОВТОРЯЮЩИХСЯ ФРАЗ-ПАРАЗИТОВ (максимум 1 раз на ВСЮ статью!):
   ...
   ❌ "— не знаю почему, но..." (удаляй полностью!)
   ❌ "— может быть, не совсем точно, но..." (удаляй полностью!)

   ❌ ПРИМЕР ЧТО НЕЛЬЗЯ:
      "Марина сказала, что придёт.вот только Она задержалась.
       — Вот в чём дело, — сказала она. — Это проблема.
       Я посмотрела на неё.вот что я хочу сказать, я была удивлена.
       — В чём проблема? — одним словом, непонятно.
       Марина посмотрела на часы.вот в чём дело, опоздала."

   ✅ ВАРЬРУЙ РЕЧЕВЫЕ ОБОРОТЫ! Используй разные фразы:
      "Я не знала..."
      "Странно, но..."
...
⚠️  БЕЗ ORPHANED ФРАГМЕНТОВ В НАЧАЛЕ АБЗАЦЕВ (КРИТИЧНО!):
   ...
   ✅ "Свет падал. Я увидела."
   ✅ "— Ты не изменилась, — мягко сказала она. — Врёшь ведь..."

⚠️  БЕЗ ОБОРВАННЫХ ПРЕДЛОЖЕНИЙ В НАЧАЛЕ:
...
⚠️  БЕЗ ПРОБЛЕМ С ПРОБЕЛАМИ:
   ❌ "Маринасиделанапротив.инедоступным,"
   ✅ "Марина сидела напротив. Она казалась недоступной."

⚠️  БЕЗ РАЗОРВАННЫХ ДИАЛОГОВ:
   ❌ Диалог начинается, потом обрывается, потом продолжается через параграф
   ✅ Если используешь диалог - делай его ПОЛНЫМ и правильно отформатированным
...
Перечитай свой текст и убедись что:

☐ НЕТ метаданных: [note], [comment], [TODO], [...] → УДАЛИ!
☐ НЕТ markdown: **, ##, ___, \`\`\` → УДАЛИ!
☐ НЕТ повторяющихся фраз > 1 раза:
   - "— вот в чём дело" → макс 1 раз
   - "— одним словом" → макс 1 раз
   - "— вот что я хочу сказать" → макс 1 раз
   - "— может быть, не совсем точно, но" → УДАЛИ полностью!
...
☐ НЕТ разорванных предложений:
   - "хотя...", "потому что..." в начале → ИСПРАВИТЬ!
☐ Диалоги ПОЛНЫЕ и правильно отформатированные
...
Если что-то найдёшь - ПЕРЕДЕЛАЙ СЕЙЧАС перед ответом!
