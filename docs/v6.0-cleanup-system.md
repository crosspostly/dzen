# 🧹 v6.0: 3-Level Article Cleanup System
...
### УРОВЕНЬ 2: AI ФИНАЛЬНАЯ ОЧИСТКА (FinalArticleCleanupGate)

**Файл:** `services/finalArticleCleanupGate.ts` (НОВЫЙ)
...
// 2. Очистка через Gemini (если нужно)
...
2. **cleanupAndValidate()** - AI очистка если severity >= threshold
   - Отправляет в Gemini с промптом "Переписать без мусора"
