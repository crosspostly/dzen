...
/**
 * Скрипт для точного анализа редактора новой статьи Дзен
 */

class DzenNewArticleAnalyzer {
  constructor() {
    this.browser = null;
    this.page = null;
  }
  ...
  async analyzeNewArticleEditor() {
    console.log('🌐 Переход к созданию новой статьи...');
    
    // Сначала переходим на главную страницу профиля
    ...
    // Ищем кнопку "Создать публикацию" или "Создать статью"
    ...
    for (const selector of createSelectors) {
      try {
        ...
        if (element && await element.isVisible()) {
          ...
          if (newUrl.includes('editor') && !newUrl.includes('edit')) { // Убедимся, что это новый редактор, а не редактирование
            console.log(`✅ Найден редактор новой статьи: ${newUrl}`);
    ...
    if (!foundCreateButton) {
      console.log('❌ Не удалось найти кнопку создания новой статьи на главной странице');
      
      // Попробуем альтернативные пути
      const alternativeUrls = [
        'https://dzen.ru/create',
        'https://dzen.ru/create/article',
        'https://dzen.ru/new',
        'https://dzen.ru/write',
        'https://dzen.ru/add',
        'https://dzen.ru/create?type=article'
      ];
      
      for (const url of alternativeUrls) {
        try {
          console.log(`\n🔍 Проверка альтернативного URL: ${url}`);
          ...
          if (currentUrl.includes('editor') && !currentUrl.includes('edit')) {
            console.log(`✅ Найден редактор новой статьи по альтернативному URL: ${currentUrl}`);
            foundCreateButton = true;
            break;
          }
        } catch (e) {
          console.log(`⚠️ Ошибка при переходе по ${url}: ${e.message}`);
          continue;
        }
      }
    }
    ...
    if (!foundCreateButton) {
      console.log('❌ Не удалось найти редактор новой статьи ни по одному из путей');
      console.log('ℹ️ Попробуем использовать прямой URL создания статьи');
      
      // Используем известный URL для создания новой статьи
      await this.page.goto('https://dzen.ru/create/article', { waitUntil: 'networkidle', timeout: 30000 });
      await this.page.waitForTimeout(10000);
    }
    
    // Теперь анализируем страницу нового редактора
    await this.page.waitForTimeout(5000);
    
    console.log('📊 Анализ структуры редактора новой статьи...');
    
    // Выполняем анализ структуры страницы
      ...
      allElements.forEach(el => {
        ...
        if (tagName === 'input' || tagName === 'textarea' || el.contentEditable === 'true') {
          ...
          const isTitleField = (
            ...
            elementInfo.textContent.toLowerCase().includes('название') ||
            elementInfo.textContent.toLowerCase().includes('name') ||
            ...
            elementInfo.className.toLowerCase().includes('input-title') ||
            elementInfo.className.toLowerCase().includes('title-input') ||
            elementInfo.className.toLowerCase().includes('article-title') ||
            elementInfo.className.toLowerCase().includes('post-title')
          );
          ...
          const isContentField = (
            ...
            elementInfo.className.toLowerCase().includes('public-DraftEditor-content') ||
            elementInfo.className.toLowerCase().includes('notranslate') ||
            ...
            elementInfo.attributes.role === 'textbox' ||
        ...
        if (tagName === 'button' || 
          ...
          const isPublishButton = (
            ...
            elementInfo.attributes['data-testid']?.toLowerCase().includes('create') ||
            elementInfo.attributes['data-testid']?.toLowerCase().includes('add') ||
            ...
            elementInfo.id?.toLowerCase().includes('create') ||
            elementInfo.id?.toLowerCase().includes('add') ||
            ...
            elementInfo.className.toLowerCase().includes('create') ||
            elementInfo.className.toLowerCase().includes('add') ||
            ...
            elementInfo.className.toLowerCase().includes('publish-button')
          );
        ...
        if (tagName === 'div') {
          ...
          if (elementInfo.className.toLowerCase().includes('modal') ||
              ...
              elementInfo.className.toLowerCase().includes('ReactModal') ||
              elementInfo.attributes.role === 'dialog' ||
              elementInfo.attributes['aria-modal'] === 'true') {
        ...
        if (tagName === 'input' && el.type === 'file') {
          ...
          if (elementInfo.attributes.accept?.toLowerCase().includes('image') ||
              ...
              elementInfo.className.toLowerCase().includes('file') ||
              elementInfo.className.toLowerCase().includes('picture') ||
              elementInfo.className.toLowerCase().includes('img'))
          {
            elements.imageUploads.push(elementInfo);
          }
        }
  ...
  generatePreciseSelectors(elementInfo) {
    ...
    // ContentEditable селектор
    if (elementInfo.attributes.contenteditable) {
      selectors.push(`[contenteditable="${elementInfo.attributes.contenteditable}"]`);
    }
    ...
    if (elementInfo.textContent && elementInfo.tagName === 'button') {
      const buttonText = elementInfo.textContent.substring(0, Math.min(30, elementInfo.textContent.length));
      if (buttonText) {
  ...
  async analyzeAndSave() {
    try {
      ...
      const structure = await this.analyzeNewArticleEditor();
      ...
      const report = {
        ...
        url: this.page.url(),
        navigationPath: 'https://dzen.ru/create/article',
      ...
      await fs.writeFile('dzen-new-article-analysis.json', JSON.stringify(report, null, 2), 'utf8');
      console.log('✅ Полный анализ сохранен в dzen-new-article-analysis.json');
      ...
      const selectorsForPublisher = {
        ...
        directUrl: 'https://dzen.ru/create/article',
        navigationInfo: {
          url: 'https://dzen.ru/create/article',
          description: 'Прямой URL к редактору новой статьи Дзен',
          note: 'Используйте этот URL для создания новых статей'
        }
      };
      
      await fs.writeFile('dzen-new-article-selectors.json', JSON.stringify(selectorsForPublisher, null, 2), 'utf8');
      console.log('✅ Селекторы для новой статьи сохранены в dzen-new-article-selectors.json');
...
// Запуск анализа
async function main() {
  const analyzer = new DzenNewArticleAnalyzer();
  
  try {
    const result = await analyzer.analyzeAndSave();
    console.log('\n✅ АНАЛИЗ РЕДАКТОРА НОВОЙ СТАТЬИ ЗАВЕРШЕН!');
    console.log('Созданы файлы:');
    console.log('- dzen-new-article-analysis.json - полный анализ структуры');
    console.log('- dzen-new-article-selectors.json - точные селекторы для публикатора');
  } catch (error) {
...
module.exports = DzenNewArticleAnalyzer;