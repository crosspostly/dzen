...
// Получаем корневой URL для изображений (замените на ваш GitHub Pages URL)
const BASE_URL = process.env.BASE_URL || 'https://yourusername.github.io';
...
function generateFeed() {
  ...
  const feed = new Feed({
    ...
    link: SITE_URL,
    language: 'ru',
    image: `${BASE_URL}/logo.png`, // Заглушка, замените на реальный логотип
  ...
  for (const filePath of markdownFiles) {
    try {
      ...
      const relativePath = path.relative('./articles', filePath); // Обновлено с content/articles на articles
      const fileName = path.basename(filePath, path.extname(filePath));
      ...
      if (frontmatter.image && !frontmatter.image.startsWith('http')) {
        ...
        for (const file of filesInDir) {
          ...
          if (imageExtensions.includes(fileExt)) {
            ...
            // или просто начинается с ожидаемого имени
            if (baseName.startsWith(expectedBaseName) ||
                expectedBaseName.startsWith(baseName) ||
                baseName.includes(expectedBaseName)) {
      ...
      if (frontmatter.image) {
        ...
        if (frontmatter.image.startsWith('http')) {
          ...
          let imagePath = path.join('articles', dirPath, actualImageFileName); // Обновлено с content/articles на articles
...
// Функция для перемещения "осиротевших" изображений (без соответствующих .md файлов)
function moveOrphanedImages() {
  console.log(`\n🔍 Поиск "осиротевших" изображений...`);

  const imageExtensions = ['.jpg', '.jpeg', '.png', '.gif', '.webp'];

  // Рекурсивно проходим по всем подкаталогам articles
  const processDirectory = (dir) => {
    const items = fs.readdirSync(dir);

    for (const item of items) {
      // Пропускаем папку published
      if (item === 'published') {
        continue;
      }
      ...
      if (stat.isDirectory()) {
        ...
        if (imageExtensions.includes(ext)) {
          ...
          // Проверяем, есть ли соответствующий .md файл
          const mdFile = path.join(path.dirname(fullPath), baseName + '.md');

          if (!fs.existsSync(mdFile)) {
            // Нашли "осиротевшее" изображение - перемещаем в published
            const relativePath = path.relative('./articles', fullPath);
            const publishedPath = path.join('./articles/published', relativePath);
            const publishedDir = path.dirname(publishedPath);
            ...
            // Перемещаем файл
            fs.renameSync(fullPath, publishedPath);
            console.log(`   🖼️  Перемещено "осиротевшее" изображение в published: ${relativePath}`);
          }
        }
      }
    }
  ...
  processDirectory('./articles');
}

// Запускаем генерацию
generateFeed();

// Перемещаем "осиротевшие" изображения после генерации ленты
moveOrphanedImages();